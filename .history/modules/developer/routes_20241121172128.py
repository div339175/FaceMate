...
def save_photo():
        ...
        # Capture sample photo using OpenCV
    ...
    student_folder = os.path.join('static/uploads', f'{roll_number}_{name}')
    ...
    # Capture the photo samples using the webcam
    count = photo_count
    while count < 20:  # Capture 20 images for training
        ...
        for (x, y, w, h) in faces:
            ...
            photo_filename = f"{roll_number}_{name}_{count}.jpg"
            photo_path = os.path.join(photo_folder, photo_filename)
            cv2.imwrite(photo_path, face_sample)

            # Draw rectangle around face for visualization
            cv2.rectangle(frame, (x, y), (x + w, y + h), (0, 255, 0), 2)

        cv2.imshow("Capture Sample Photo", frame)
...
def train_classifier():
    ...
    for folder in student_folders:
        student_folder = os.path.join('static/uploads', folder)
        photo_folder = os.path.join(student_folder, 'photo_samples')
    ...
    classifier_path = os.path.join('modules/static/uploads', 'classifier.xml')
