...
def save_photo():
    ...
    try:
        ...
        if img is None:
            return jsonify({"message": "Failed to decode image. Possible corrupt JPEG data."}), 400
        ...
        # Create a folder for each user to store their photos
        user_folder = f"static/uploads/{user_id}"
        os.makedirs(user_folder, exist_ok=True)

        # Save photo sample
        photo_path = f"{user_folder}/photo_{len(os.listdir(user_folder))}.jpg"
        cv2.imwrite(photo_path, img)
    
        return jsonify({"message": "Photo saved successfully"})

    except Exception as e:
        return jsonify({"message": f"Error processing photo: {str(e)}"}), 500
    ...
    try:
        ...
        for user_id in user_folders:
            ...
            if os.path.isdir(user_path):
                for photo_file in os.listdir(user_path):
                    img_path = os.path.join(user_path, photo_file)
                    img = cv2.imread(img_path, cv2.IMREAD_GRAYSCALE)
                    faces = face_cascade.detectMultiScale(img, scaleFactor=1.1, minNeighbors=5)

                    for (x, y, w, h) in faces:
                        face_samples.append(img[y:y + h, x:x + w])
                        labels.append(int(user_id))  # Assuming user IDs are integers

        if not face_samples:
            return jsonify({"message": "No faces found for training."})

        # Train the model with all collected samples
        face_recognizer.train(face_samples, np.array(labels))

        # Save the trained model to a file for future recognition
        model_path = 'static/classifier.xml'
...
def recognize_face():
    try:
        ...
        if not photo_data:
            return jsonify({"message": "Photo data is missing"}), 400
        ...
        for (x, y, w, h) in faces:
            ...
            # Predict the label (user ID) for the detected face
            label, confidence = face_recognizer.predict(face)

            # Optional: Set a threshold for confidence to consider the face recognized
            if confidence < 100:
                return jsonify({"message": f"Face recognized! User ID: {label}", "userId": label})
            else:
