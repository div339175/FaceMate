...
@app.route('/developer/save-photo', methods=['POST'])
def save_photo():
    ...
    photo_path = os.path.join(photo_folder, f"{roll_no}_{len(os.listdir(photo_folder)) + 1}.jpg")
    cv2.imwrite(photo_path, img)
    
    return jsonify({'message': 'Photo saved successfully.'}), 200

# Route to train the model
@app.route('/developer/train-data', methods=['POST'])
def train_data():
    recognizer = cv2.face.LBPHFaceRecognizer_create()
