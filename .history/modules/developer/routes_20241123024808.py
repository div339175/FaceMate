...
def train_data():
    ...
    base_path = os.path.abspath(f"modules/developer/static/uploads/{roll_no}"/'photo_samples')
    photo_samples_path = os.path.join(base_path, "photo_samples")

    if not os.path.exists(photo_samples_path):
        raise FileNotFoundError(f"Directory does not exist: {photo_samples_path}")
    ...
    if faces and labels:
        ...
        model_path = os.path.join(photo_samples_path, roll_no)
