...
def register_student():
    ...
    if request.method == 'POST':
        ...
        courses = request.form.getlist('courses') # Assuming you are passing a list of course IDs
 ...
 # Hash the password
       ...
       # Save user information to the User table
        ...
        for course_id in courses:
            # Create an entry in the StudentCourse association table
            student_course = StudentCourse(student_id=student.id, course_id=course_id)
            db.session.add(student_course)

        db.session.commit()  # Commit to save all the data
        ...
        #     db.session.commit()
            ...
            logout_user()
            return redirect(url_for('developer.developer_dashboard',student_id=student.id))
    
    return render_template('register_student.html')


@developer_bp.route('/get_courses', methods=['GET'])
def get_courses():
    query = request.args.get('query', '')  # Get the search query
    if query:
        # Fetch courses where the name or code matches the query
