...
def train_classifier():
    ...
    # Save the trained model to a unique file for each student
for roll_number, label in label_dict.items():
    classifier_path = os.path.join('modules/developer/static/uploads', f'classifier_{roll_number}.xml')
    # Train the recognizer only for the current student's data
    student_faces = [faces[i] for i in range(len(labels)) if labels[i] == label]
    student_labels = [label] * len(student_faces)
    if student_faces:
        recognizer.train(student_faces, np.array(student_labels))
