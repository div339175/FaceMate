...
import base64
import numpy as np
from models import Student,db,AttendanceSession,Attendance,StudentCourse
...
def student_dashboard():
    ...
    sd = StudentCourse.query.filter_by(roll_number=roll_number).first()
...
# Check if the student has any courses
    if student and sd.course_code:
        active_sessions = []

        # Iterate over each course the student is enrolled in
        for course in sd.course_code:
            # Get the active attendance session for this course
            active_session = get_active_session(course)
            
            if active_session:
                active_sessions.append(active_session)
        
    return render_template('student_dashboard.html',student=student,active_session=active_session)
