...
import base64
import numpy as np
from models import Student,db,AttendanceSession,at
...
@login_required
def recognize_face():
    student_id = current_user.id  # Authenticated student
    ...
    # Perform face recognition (assume face_recognizer() is a helper function)
    is_recognized = face_recognizer(current_user.face_embedding)
    status = 'Present' if is_recognized else 'Absent'

    # Save the result in attendance records
    record = AttendanceRecord(
        session_id=session_id,
        student_id=student_id,
        status=status,
        recognized_time=datetime.utcnow() if is_recognized else None
    )
    db.session.add(record)
    db.session.commit()

    return jsonify({'message': 'Attendance marked', 'status': status})
