...
def update_csv_attendance(csv_file, roll_no):
    ...
    # Check if the student's roll number is present
    student_row = df[df["Roll Number"] == roll_no]
    if student_row.empty:
        print(f"Student {roll_no} not found in CSV.")
        return False

    # Update attendance status
    df.loc[df["Roll Number"] == roll_no, "Attendance Status"] = "Present"
    df.to_csv(csv_file, index=False)  # Save the updated CSV
    print(f"Attendance for Roll No {roll_no} marked as Present.")
    return True

def normalize_roll_no(roll_no, length=4):
    """Pad the roll number with leading zeros to ensure it's a fixed length"""
    return str(roll_no).zfill(length)

# Normalize the roll number when comparing
