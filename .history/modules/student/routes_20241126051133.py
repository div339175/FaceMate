...
def create_csv(session_id, batch_id, course_id):
    file_name = f"attendance_session_{session_id}_{batch_id}_{course_id}.csv"
    file_path = os.path.join('path_to_your_directory', file_name)  # Ensure the correct path
    ...
    # Create CSV with initial student list (if necessary)
    header = ["Roll Number", "Student Name", "Attendance Status"]
...
def update_csv_attendance(csv_file, roll_no):
    ...
    # Convert roll_no to integer to ignore leading zeros
    roll_no_normalized = normalize_roll_no(roll_no)
    if roll_no_normalized is None:
        return False  # Invalid roll_no passed
    ...
    # Ensure that the 'Roll Number' column is interpreted as integer for consistent comparison
    df['Roll Number'] = df['Roll Number'].astype(int)

    # Debugging: Print the available roll numbers in the CSV file
    print(f"Available roll numbers in CSV: {df['Roll Number'].tolist()}")
    print(f"Comparing with roll_no_normalized: {roll_no_normalized}")

    # Update attendance status for the student
    student_row = df[df["Roll Number"] == roll_no_normalized]
    if not student_row.empty:
        print(f"Found student: {student_row}")
        df.loc[df["Roll Number"] == roll_no_normalized, "Attendance Status"] = "Present"
        df.to_csv(csv_file, index=False)  # Save the updated CSV
        return True
    else:
        print("Student not found in CSV")
        return False
...
# Get the student record based on roll number and logged-in user email
        ...
        csv_file = f"attendance_session_{active_session.session_id}_{student.batch}_{active_session.course_id}.csv"
        ...
        for (x, y, w, h) in faces:
            ...
            if confidence < 60 and label == int(roll_no):  # Directly compare with roll_no
