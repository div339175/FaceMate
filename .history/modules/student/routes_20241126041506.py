...
def recognize_faces():    
    try:
        ...
        # Find the correct CSV file based on session
...
def recognize_faces():
    try:
        # Get the active session for the student
        student = Student.query.filter_by(email=current_user.email).first()
        if not student:
            return jsonify({"message": "Student not found"}), 404
        ...
        if not active_session:
            return jsonify({"message": "No active session found for your batch and course."}), 400

        # Extract session_id and batch_id from the active session
        session_id = active_session.session_id
        batch_id = student.batch

        # Now call the function to handle face recognition and attendance update
        data = request.get_json()
        photo_data = data.get('photoData')
        roll_no = data.get('rollNO')
        ...
        # Call the function that processes the image and updates attendance
        csv_file = create_csv(session_id, batch_id)  # Create the CSV file (if not already present)
        
        # Process the face recognition and attendance update
        return process_face_recognition_and_update(csv_file, roll_no, session_id)

    except Exception as e:
        return jsonify({"message": f"Error: {str(e)}"}), 500


def process_face_recognition_and_update(csv_file, roll_no, session_id):
    # Load Haar Cascade for face detection
    cascade_path = 'haarcascade_frontalface_default.xml'
    face_cascade = cv2.CascadeClassifier(cascade_path)
