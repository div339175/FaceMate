...
def save_attendance():
    try:
        ...
        course_id = data.get('course_id')  # Use course_id instead of batch_id

        # Construct the path to the CSV file
        csv_file = os.path.join("attendance_files", f"attendance_session_{course_id}.csv")
...
ed on the received data
        roll_no_to_attendance 
        # Update the attendance status bas= {str(item['roll_no']): item['attendance'] for item in attendance_data}
...
@teacher_bp.route('/download_csv', methods=['GET'])
def download_csv():
    course_id = request.args.get('course_id')
    csv_file = os.path.join("attendance_files", f"attendance_batch_{course_id}.csv")
