...
from models import User,Teacher,Course,Student,db,AttendanceSession,Attendance
...
def start_session():
 ...
 # Get the course object
    course = Course.query.get(course_i)

    if not course:
        return jsonify({'error': 'Invalid course'}), 400

    # Get students who are enrolled in the selected course and belong to the selected batch
    students_in_course_and_batch = Student.query.filter(
        Student.courses.any(Course.id == course.id),  # Check if student is enrolled in the selected course
        Student.batch_id == batch_id  # Check if student belongs to the selected batch
    ).all()

    if not students_in_course_and_batch:
        return jsonify({'error': 'No students found for this course and batch'}), 400
...
def save_attendance():
    ...
    for record in data:
        ...
        # Get the course object
            ...
            for student in students_in_course_and_batch:                # Record attendance for this student in this session
