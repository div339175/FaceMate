...
def view_attendance():
    ...
    try:
        ...
        return render_template('view_attendance.html', students=students, batch_id=batch_id)
    except Exception as e:
        return jsonify({"error": "Failed to read CSV file", "details": str(e)}), 500import logging
...
# Modify the existing code to include logs
@teacher_bp.route('/login', methods=['GET', 'POST'])
def login():
    if request.method == 'POST':
        email = request.form['email']
        ...
        if user and user.password == password:
            login_user(user)
            logger.info(f'Teacher {email} logged in successfully')
            return redirect(url_for('teacher.teacher_dashboard'))
        logger.error(f'Invalid login attempt by {email}')
        flash('Invalid email or password', 'danger')
...
def save_attendance():
    try:
        ...
        # Update the attendance based on the submitted data
        for student in students:
            for attendance_entry in attendance_data:
                if student['roll_no'] == attendance_entry['roll_no']:
                    student['attendance'] = attendance_entry['attendance']

        # Write the updated data back to the CSV file
        with open(csv_file, mode='w', newline='', encoding='utf-8') as file:
            fieldnames = ['roll_no', 'name', 'attendance', 'photo_path']
