...
def view_attendance():
 ...
 # Query to get students in the given batch and course
    students = db.session.query(Student, Attendance).join(Attendance, Student.id == Attendance.student_id).filter(
        Student.batch_id == batch_id, Student.course_code == course_code).all()
