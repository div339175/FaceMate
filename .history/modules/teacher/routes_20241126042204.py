...
def start_session():
    try:
        ...
        session_id = create_session(batch_id, course_id)  # Assume you have a function to create the session
        csv_file = create_csv(session_id, batch_id, course_id)

    flash("Session started and attendance CSV created.", "success")
...
def create_csv(session_id, batch_id):
    """
    Creates a CSV file for the given session with attendance data for the students in the batch.
    :param session_id: ID of the attendance session
    :param batch_id: Batch ID for which the session is being created
    """
    try:
        # Define the file path for the CSV
        csv_dir = "attendance_files"
        os.makedirs(csv_dir, exist_ok=True)  # Ensure the directory exists
        csv_file = os.path.join(csv_dir, f"attendance_session_{session_id}.csv")

        # Fetch students in the batch
        students = Student.query.filter_by(batch=batch_id).all()
        if not students:
            raise ValueError("No students found for the given batch.")

        # Define the headers for the CSV file
        headers = ["Roll Number", "Name", "Batch", "Photo", "Attendance Status"]
        ...
        with open(csv_file, mode='w', newline='', encoding='utf-8') as file:
            ...
            # Write student data with default attendance status (Absent)
            for student in students:
                writer.writerow([
                    student.roll_number,
                    student.name,
                    student.batch,
                    student.photo_url if hasattr(student, 'photo_url') else "N/A",  # Photo URL or placeholder
