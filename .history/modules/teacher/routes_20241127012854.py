...
# Add this route to handle the view attendance page in Flask
@teacher_bp.route('/view_attendance', methods=['GET'])
def view_attendance():
    batch_id = request.args.get('batch_id')
    course_code = request.args.get('course_code')

    # If batch_id or course_code is missing, return an error or redirect to the dashboard
    if not batch_id or not course_code:
        return redirect('/teacher/dashboard')  # Or show an error message

    # Fetch students based on the batch_id and course_code
    students = Student.query.join(StudentCourse, Student.roll_number == StudentCourse.roll_number).filter(
        StudentCourse.course_code == course_code,
        Student.batch == batch_id
    ).all()

    # Pass students data to the HTML page
